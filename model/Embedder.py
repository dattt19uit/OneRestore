...
class HybridEmbedder(Embedder):
    def __init__(self, *args, **kwargs):
        ...
        # OpenAI client (API key lấy từ env)
        if "OPENAI_API_KEY" not in os.environ:
            raise ValueError("❌ OPENAI_API_KEY not found in environment variables.")
        self.openai_client = OpenAI()

    # ===============================
    # New: Explain prediction with LLM
    # ===============================
    def explain_prediction_with_llm(self, pil_image, fusion="mean"):
        """
        1. Predict degradation class (static + dynamic fusion).
        2. Call LLM để giải thích tại sao ảnh thuộc class đó.
        """

        print("explain_prediction_with_llm - Start")
        ...
        # Image features
        img_feat = self.feat_extractor(img_tensor)[0]
        img = self.img_embedder(img_feat)
        img = self.img_avg_pool(img).squeeze(3).squeeze(2)
        img = self.img_final(img)

        # Classify
        pred = self.classifier(img, static_weight)
        pred_idx = torch.max(pred, dim=1)[1].item()
        pred_class = self.type_name[self.train_type[pred_idx].item()]
        print(f"explain_prediction_with_llm - Predicted class: {pred_class}")

        # ----------------------------
        # Step 2. Generate detailed description (dynamic, BLIP)
        # ----------------------------
        description = self.generate_degradation_description(pil_image)
        print(f"explain_prediction_with_llm - BLIP description: {description}")

        # ----------------------------
        # Step 3. Call LLM for reasoning
        # ----------------------------
        prompt = f"""
        I have an image predicted as "{pred_class}" (a type of weather degradation).
        Supporting information:
        - BLIP detailed description: "{description}".
        - This class comes from a taxonomy of 12 degradations (clear, low light, haze, rain, snow, and their composites).

        Please explain in simple English:
        1. Why this image is likely categorized as "{pred_class}".
        2. Which visual cues or features usually indicate "{pred_class}".
        3. How "{pred_class}" differs from other degradations.
        """

        response = self.openai_client.chat.completions.create(
            model="gpt-4o-mini",   # bạn có thể đổi sang gpt-4.1 / gpt-5
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7
        )
        explanation = response.choices[0].message.content

        print(f"explain_prediction_with_llm - LLM explanation: {explanation}")

        return pred_class, description, explanation
