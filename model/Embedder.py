...
class Backbone(nn.Module, PyTorchModelHubMixin, repo_url="https://github.com/gy65896/OneRestore", pipeline_tag="image-feature-extraction"):
    ...
    def forward(self, x, returned=[4]):
        logging.debug(f"Backbone forward input shape: {x.shape}")
        ...
        logging.debug(f"Backbone forward output shapes: {[o.shape for o in out]}")
        return out
...
class CosineClassifier(nn.Module):
    ...
    def forward(self, img, concept, scale=True):
        logging.debug(f"CosineClassifier forward - img shape: {img.shape}, concept shape: {concept.shape}")
        ...
        if scale:
            pred = pred / self.temp
        logging.debug(f"CosineClassifier predictions shape: {pred.shape}")
        return pred
...
class Embedder(nn.Module):
    ...
    def train_forward(self, batch):
        ...
        bs = img.shape[0]
        logging.debug(f"train_forward - Batch size: {bs}, Scene shape: {scene.shape}, Image shape: {img.shape}")

        # word embedding
        scene_emb = self.embedder(self.train_type)
        scene_weight = self.mlp(scene_emb)
        logging.debug(f"train_forward - Scene embedding shape: {scene_emb.shape}, Scene weight shape: {scene_weight.shape}")
        ...
        logging.debug(f"train_forward - Image final embedding shape: {img.shape}")
        ...
        logging.debug(f"train_forward - Predictions: {pred}, Label loss: {label_loss.item()}, Accuracy: {correct_type.sum() / float(bs)}")
...
class HybridEmbedder(Embedder):
    ...
    def generate_degradation_description(self, pil_image):
        ...
        """
        logging.debug("generate_degradation_description - Starting description generation")
        prompt = (
        ...
        logging.debug(f"generate_degradation_description - Generated description: {description}")
        return description
    ...
    def encode_dynamic_text(self, texts):
        ...
        """
        logging.debug(f"encode_dynamic_text - Encoding texts: {texts}")
        ...
        logging.debug(f"encode_dynamic_text - Encoded embedding shape: {projected_emb.shape}")
    ...
    def train_forward(self, batch, dynamic_texts=None, fusion="mean"):
        ...
        scene, img = batch[0], self.transform(batch[1])
        bs = img.shape[0]
        logging.debug(f"train_forward - Batch size: {bs}, Dynamic texts provided: {dynamic_texts is not None}, Fusion mode: {fusion}")

        # --- Static embedding
        scene_emb = self.embedder(self.train_type)
        static_weight = self.mlp(scene_emb)
        logging.debug(f"train_forward - Static weight shape: {static_weight.shape}")

        # --- Dynamic embedding
        if dynamic_texts is not None:
            dyn_weight = self.encode_dynamic_text(dynamic_texts)
            logging.debug(f"train_forward - Dynamic weight shape: {dyn_weight.shape}")
            if fusion == "mean":
            ...
            logging.debug(f"train_forward - Fused scene weight shape: {scene_weight.shape}")
        else:
            scene_weight = static_weight
        ...
        img = self.img_avg_pool(img).squeeze(3).squeeze(2)
        img = self.img_final(img)
        logging.debug(f"train_forward - Image embedding shape: {img.shape}")
        ...
        type_pred = self.train_type[pred]
        correct_type = (type_pred == scene)
        logging.debug(f"train_forward - Predictions: {pred}, Type predictions: {type_pred}, Label loss: {label_loss.item()}, Accuracy: {correct_type.sum() / float(bs)}")
    ...
    def forward(self, x, mode='image_encoder', dynamic_texts=None, fusion="mean"):
        logging.debug(f"forward - Mode: {mode}")
        if mode == 'train':
    ...
    def image_encoder_forward(self, batch):
        ...
        """
        logging.debug("image_encoder_forward - Starting")
        # word embedding
        scene_emb = self.embedder(self.train_type)
        scene_weight = self.mlp(scene_emb)
        logging.debug(f"image_encoder_forward - Scene weight shape: {scene_weight.shape}")
        ...
        img = self.img_avg_pool(img).squeeze(3).squeeze(2)
        img = self.img_final(img)
        logging.debug(f"image_encoder_forward - Image embedding shape: {img.shape}")

        pred = self.classifier(img, scene_weight)
        pred = torch.max(pred, dim=1)[1]
        logging.debug(f"image_encoder_forward - Predictions: {pred}")
        ...
        logging.debug(f"image_encoder_forward - Output embedding shape: {out_embedding.shape}, Num types: {num_type}, Text types: {text_type}")
    ...
    def enhanced_image_encoder_forward(self, pil_images):
        ...
        """
        logging.debug("enhanced_image_encoder_forward - Starting")
        bs = len(pil_images)
        device = "cuda" if torch.cuda.is_available() else "cpu"
        logging.debug(f"enhanced_image_encoder_forward - Batch size: {bs}, Device: {device}")
        ...
        logging.debug(f"enhanced_image_encoder_forward - Image tensors shape: {img_tensors.shape}")

        # Static: Word embedding and classification
        scene_emb = self.embedder(self.train_type.to(device))
        scene_weight = self.mlp(scene_emb)
        logging.debug(f"enhanced_image_encoder_forward - Scene weight shape: {scene_weight.shape}")
        ...
        img = self.img_avg_pool(img).squeeze(3).squeeze(2)
        img = self.img_final(img)
        logging.debug(f"enhanced_image_encoder_forward - Image final shape: {img.shape}")

        pred = self.classifier(img, scene_weight)
        pred_idx = torch.max(pred, dim=1)[1]
        type_pred = self.train_type.to(device)[pred_idx]
        text_type = [self.type_name[type_pred[i].item()] for i in range(bs)]
        logging.debug(f"enhanced_image_encoder_forward - Predictions indices: {pred_idx}, Type predictions: {type_pred}, Text types: {text_type}")
        ...
        for i in range(bs):
            ...
            logging.debug(f"enhanced_image_encoder_forward - Image {i}: Static embedding for type {self.type_name[static_idx]}")
            ...
            logging.debug(f"enhanced_image_encoder_forward - Image {i}: Dynamic embedding generated")

            # Average vector for static + dynamic
            enhanced_emb = (static_emb + dyn_emb) / 2
            out_embedding[i] = enhanced_emb
            logging.debug(f"enhanced_image_encoder_forward - Image {i}: Enhanced embedding computed")
    ...
    def text_encoder_forward(self, text):
        ...
        bs = len(text)
        logging.debug(f"text_encoder_forward - Batch size: {bs}, Texts: {text}")

        # word embedding
        scene_emb = self.embedder(self.train_type)
        scene_weight = self.mlp(scene_emb)
        logging.debug(f"text_encoder_forward - Scene weight shape: {scene_weight.shape}")
        ...
        text_type = text
        logging.debug(f"text_encoder_forward - Output embedding shape: {out_embedding.shape}, Num types: {num_type}, Text types: {text_type}")
    ...
    def text_idx_encoder_forward(self, idx):
        ...
        bs = idx.shape[0]
        logging.debug(f"text_idx_encoder_forward - Batch size: {bs}, Indices: {idx}")

        # word embedding
        scene_emb = self.embedder(self.train_type)
        scene_weight = self.mlp(scene_emb)
        logging.debug(f"text_idx_encoder_forward - Scene weight shape: {scene_weight.shape}")
        ...
        for i in range(bs):
            out_embedding[i,:] = scene_weight[int(num_type[i]),:]
        logging.debug(f"text_idx_encoder_forward - Output embedding shape: {out_embedding.shape}")

        return out_embedding

    def contrast_loss_forward(self, batch):

        logging.debug("contrast_loss_forward - Starting")
        ...
        img = self.img_avg_pool(img).squeeze(3).squeeze(2)
        img = self.img_final(img)
        logging.debug(f"contrast_loss_forward - Output shape: {img.shape}")

        return img
    
    def forward(self, x, type = 'image_encoder'):

        logging.debug(f"forward - Type: {type}")
        if type == 'train':
        ...
        # logging.debug("forward - Completed with output type: ", type(out))
